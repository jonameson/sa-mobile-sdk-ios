...
@property (nonatomic, assign) BOOL is2sHandled;
...
- (id) init {
    if (self = [super init]) {
        ...
        _eventHandler = ^(SAVideoPlayerEvent event, CGFloat time, CGFloat duration) {};
        _clickHandler = ^(){};
    }
    return self;
}
...
- (void) setEventHandler:(saVideoPlayerDidReceiveEvent) handler {
    _eventHandler = handler != NULL ? handler : ^(SAVideoPlayerEvent event, CGFloat time, CGFloat duration) {};
}
        ...
        if (_playerItem.status == AVPlayerItemStatusReadyToPlay) {
            ...
            _observer = [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 2) queue:dispatch_get_main_queue() usingBlock:^(CMTime currentTime) {
                ...
                if (time >= 1 && !weakSelf.isStartHandled) {
                    ...
                    weakSelf.eventHandler(Video_Start, time, duration);
                };
                
                if (time >= 2.0f && !weakSelf.is2sHandled) {
                    weakSelf.is2sHandled = true;
                    weakSelf.eventHandler(Video_2s, time, duration);
                }
                
                if (time >= (duration / 4) && !weakSelf.isFirstQuartileHandled){
                    weakSelf.isFirstQuartileHandled = true;
                    weakSelf.eventHandler(Video_1_4, time, duration);
                }
                
                if (time >= (duration / 2) && !weakSelf.isMidpointHandled){
                    weakSelf.isMidpointHandled = true;
                    weakSelf.eventHandler(Video_1_2, time, duration);
                }
                
                if (time >= (3 * duration / 4) && !weakSelf.isThirdQuartileHandled) {
                    weakSelf.isThirdQuartileHandled = true;
                    weakSelf.eventHandler(Video_3_4, time, duration);
                }
                
                if (time >= 15.0f && !weakSelf.is15sHandled) {
                    weakSelf.is15sHandled = true;
                    weakSelf.eventHandler(Video_15s, time, duration);
                }
            }];
        }
